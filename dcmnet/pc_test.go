...
func TestPCsSingleAcceptance(t *testing.T) {
	if got := simplePcs.FindAcceptedTCap(PCID(1)); !reflect.DeepEqual(&matchingTc, got) {
		t.Errorf("unknown tc for pcid 1: %s", got)
	}
	expectMatchingTc(t, simplePcs.FindAcceptedTCap(PCID(1)))

	if got := simplePcs.FindAcceptedTCap(PCID(2)); got != nil {
		t.Errorf("expected no tc for pcid 2 but got: %s", got)
	}

	if got := simplePcs.FindAcceptedPCID(matchingTc); got == nil || PCID(1) != *got {
		t.Errorf("unexpected pcid: %d", got)
	}

	nonMatchingTc := matchingTc
	nonMatchingTc.AbstractSyntax = "1.2.3.4"
	if got := simplePcs.FindAcceptedPCID(nonMatchingTc); got != nil {
